...
pointing at it is not (`.Codex/*` is gitignored), so a fresh clone has the
rule without the enforcement until a settings file names the script. Put
that entry in `~/.Codex/settings.json` rather than per-worktree — every
worktree gets its own `.Codex/`, and one user-level entry covers all of

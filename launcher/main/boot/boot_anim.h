 ...
 * three seconds of frame loop, and boot_anim_curve.h is the curve itself,
...
/* Everything boot_anim_screen_y() needs for one frame, computed once rather
 * than once per point: the rotated screen-space weights for Re/Im/t, all Q8
 * like BOOT_ANIM_RE_X was, and the frame's origin. */
typedef struct {
    int32_t re_y, im_y, t_y;   /* Q8 - see boot_anim_screen_y() */
    int     oy;                 /* pixels */
} boot_anim_view_t;

/* The view for a given fraction of the climb, `progress_q12` running 0 (the
 * side-on starting view) to BOOT_ANIM_ONE (fully tilted). It is a fraction of
 * the CLIMB rather than of elapsed time on purpose - the caller drives it
 * from the same progress that paces the pen, in boot_anim.c, so the camera
 * finishes turning exactly when the curve finishes drawing rather than on an
 * independent clock that could drift against it.
 *
 * oy is the mirror-image relationship named in "THE CAMERA ORBITS": it ends
 * as far below the top of the screen as it started above the bottom. Written
 * as h - 2*oy_start rather than a second hardcoded fraction so the mirror
 * holds even if boot_anim_origin_y()'s own fraction ever changes. */
static inline boot_anim_view_t boot_anim_view(int h, int32_t progress_q12)
{
    const uint16_t phase = (uint16_t)
        (((int64_t)progress_q12 * BOOT_ANIM_PHI_END_PHASE) >> BOOT_ANIM_Q);
    ...
    v.oy = oy_start +
           (int)((((int64_t)(h - 2 * oy_start)) * progress_q12) >>
                 BOOT_ANIM_Q);
    return v;
}
...
/* value -> screen x. Unaffected by the camera orbiting, because the real
 * axis's screen direction is the pitch axis - see boot_anim_view() - and
 * rotating about an axis leaves that axis exactly where it was.
 *
 ...
 * frame counter, so the animation lasts three seconds whatever the frame rate

...
void
button_fsm_reset(button_fsm_t* b) {
    b->stable = false;
    b->candidate = false;
    b->candidate_us = 0;
    b->primed = false;
...
void
button_fsm_update(button_fsm_t* b, bool raw_down, int64_t now_us) {
    if (!b->primed) {
    ...
    if (b->stable && !b->hold_fired && now_us - b->down_since_us >= BUTTON_HOLD_US) {
        b->held = true;
        b->hold_fired = true;
    }
...
bool
button_fsm_is_down(const button_fsm_t* b) {
    return b->stable;
}

bool
button_fsm_take_pressed(button_fsm_t* b) {
...
bool
button_fsm_take_released(button_fsm_t* b) {
...
bool
button_fsm_take_held(button_fsm_t* b) {

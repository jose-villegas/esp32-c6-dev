...
static const field_row_t fields[] = {
    G(flammability), T(ignites_to), G(needs_air),
    G(burns), G(burn_decay), E(lit_from), G(residue), T(quench_to),
    G(flare), G(explodes),
    G(dissolves), G(dissolvable), G(fizz), G(evaporates),
    G(condenses), T(condenses_to),
    T(heats_to), G(heat_chance), G(melts), T(flaw_to), G(flaw_chance),
    T(spoils_to), G(spoils_chance),
    G(heat_ramp), G(cools),
    G(chills), G(conducts), G(boils), T(boils_to), G(warms), G(thaws),
    G(wets), G(soaks), T(soaks_to), G(dries), G(soil), E(tones), E(moist_max),
    T(soaked_to), G(soaked_chance),
    G(grows), G(falls), G(withers),
    T(hardens_to), G(harden_run), G(harden_chance), T(clings_to),
    G(roots), T(roots_to), T(sheltered_by),
    G(canopy), T(canopy_to), G(trunk_girth), G(holds_line),
    G(sprouts), T(sprouts_to), G(buds), T(buds_to), G(drinks),
    T(shatters_to),
};
...
int main(void)
{
    ...
    for (size_t ri = 0; ri < all_rows_count; ri++) {
        ...
        for (size_t fi = 0; fi < NFIELDS; fi++) {
            if (fields[fi].kind != KIND_GATE) continue;

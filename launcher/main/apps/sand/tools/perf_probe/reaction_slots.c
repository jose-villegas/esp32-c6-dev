...
static const char *const slot_field_names[F_COUNT] = {
    "flammability", "needs_air",
    "burns", "burn_decay", "residue", "flare", "explodes",
    "dissolves", "dissolvable", "fizz", "evaporates",
    "condenses",
    "heat_chance", "melts", "flaw_chance", "spoils_chance",
    "heat_ramp", "cools",
...
int slot_row_of(cell_t c)
{
    const uint8_t m = CELL_MATERIAL(c);
    if (m != MAT_EXTENDED) {
        if (m < MAT_SAND || m >= MAT_COUNT) return -1;
    ...
    for (int i = 0; i < slot_row_count; i++) {
        if (strcmp(slot_row_names[i], nm) == 0) return i;
    }
    return -1;
}
...
static int slot_row_of_reaction(const reaction_t *r)
{
    for (uint8_t m = MAT_SAND; m < MAT_COUNT; m++) {
        if (&reactions[m] == r) return (int)(m - MAT_SAND);
    }
    for (uint8_t k = 0; k < MATERIAL_EXTENDED_COUNT; k++) {
    ...
    for (int k = MATERIAL_EXTENDED_COUNT; k < MATERIAL_EXTENDED_CODES; k++) {
        if (&extended_reactions[k] == r) return slot_row_count - 1;
    }
    return -1;
}

...
static void emit_ignite(const reaction_t *r, uint8_t self_id)
{
    ...
    if (r->ignites_to == 0 || r->ignites_to == MAT_FIRE) {
        printf("- Catches fire %s%s.\n", adv, air);
    } else if (r->ignites_to == self_id) {
        printf("- Catches fire %s%s and burns where it stands, rather "
               "than flaring away.\n", adv, air);
    } else {
        printf("- Catches fire %s%s, charring to %s.\n", adv, air,
               prose_name(to_name(r->ignites_to)));
    }
}
...
static void emit_transform(const reaction_t *r)
{
    ...
    if (r->heat_ramp != 0) {
        ...
        printf("- Melts to %s under long heat.\n",
...
static void emit_anatomy(void)
{
     ...
     * ignites_to == self_id. */
    {
        ...
        print_example("Ignite - GRP_IGNITE: flammability, ignites_to "
                       "(emit_ignite)", segs, ARRAY_LEN(segs));
    }
     ...
     * bug had this just called prose_name() twice inline). */
    {
        const mrow_t *row = find_row("Plant");
        char hardens_to[32];
        char clings_to[32];

 ...
 * one latched. */
static void
test_the_brush_and_the_setter_agree_about_every_material(void) {
    for (int m = 1; m < MAT_COUNT; m++) {
        ...
        TEST_ASSERT_FALSE_MESSAGE(CELL_IS_EMPTY(painted), "fixture check: the brush has to actually paint the centre "
        ...
        char why[96];
        snprintf(why, sizeof why, "brush and setter disagree about %s", material_by_id((material_id_t)m)->name);
     ...
     * sand_spawn(). */
    {
        ...
        TEST_ASSERT_FALSE_MESSAGE(CELL_IS_EMPTY(painted), "fixture check: the brush has to actually paint gunpowder "
...
static void
test_snow_melts_in_any_liquid(void) {
...
static void
test_melting_snow_makes_water_not_more_of_the_liquid(void) {
    ...
    for (unsigned k = 0; k < sizeof liquids / sizeof liquids[0]; k++) {
        ...
        TEST_ASSERT_TRUE_MESSAGE(water_seen, "snow melted by oil or acid has to leave WATER behind at some "
...
static void
test_stone_heats_up_next_to_lava(void) {
    fixture();
    sand_clear(&s);
    for (int x = 0; x < W; x++) {
        sand_set(&s, x, H - 1, STONE);
...
static void
seal_water_beside(int wx, int wy) {
...
static void
test_water_cools_hot_stone_back_to_room_temperature(void) {
    fixture();
    sand_set(&s, 3, 3, CELL_MAKE(MAT_STONE, MATERIAL_VARIANTS - 1));
    seal_water_beside(4, 3);

    int wet_steps = 0;
    while (wet_steps < 2000 && CELL_VARIANT(sand_at(&s, 3, 3)) != SAND_AMBIENT_HEAT) {
        sand_step(&s, 0, 1000, 0);
        wet_steps++;
    }

    TEST_ASSERT_EQUAL_INT_MESSAGE(SAND_AMBIENT_HEAT, CELL_VARIANT(sand_at(&s, 3, 3)),
...
static void
test_water_never_chills_stone_below_room_temperature(void) {
    fixture();
    sand_set(&s, 3, 3, CELL_MAKE(MAT_STONE, MATERIAL_VARIANTS - 1));
    seal_water_beside(4, 3);
...
static void
test_stone_never_melts_however_hot(void) {
...
static void
test_snow_cracks_glass_but_not_stone(void) {
    fixture();
    sand_clear(&s);
    const int mid = W / 2;
    for (int x = 0; x < W; x++) {
...
test_an_edge_shows_less_temperature_than_the_body(void) {
    ...
    for (unsigned k = 0; k < sizeof tempered / sizeof tempered[0]; k++) {
        ...
        TEST_ASSERT_TRUE_MESSAGE(colour_gap(rest_edge, hot_edge) < colour_gap(rest_body, hot_body), why);
    ...
    for (int m = 1; m < MAT_COUNT; m++) {
        for (int v = 0; v < MATERIAL_VARIANTS; v++) {
            ...
            if (m == MAT_STONE || m == MAT_GLASS) {
                ...
                TEST_ASSERT_EQUAL_MESSAGE(pal[CELL_MAKE(m, MASS_MAX)], col[0], why);
...
test_stone_speckles_by_position_at_every_temperature(void) {
    for (int v = 0; v < MATERIAL_VARIANTS; v++) {
        ...
        snprintf(why, sizeof why,
                 ...
                 "one shade is the flat slab this exists to undo",
                 v);
...
test_cullet_shades_are_four_distinct_tints(void) {
    ...
    for (int i = 0; i < SAND_CULLET_SHADES; i++) {
        for (int j = i + 1; j < SAND_CULLET_SHADES; j++) {
            char why[80];
            snprintf(why, sizeof why, "cullet shade %d must differ from shade %d at phase 0", i, j);
...
test_a_cullet_glint_is_pure_white(void) {
    ...
    TEST_ASSERT_TRUE_MESSAGE(hash < 4096u, "no hash in 0..4095 made cullet shade 12 glint at phase 0 - is the "

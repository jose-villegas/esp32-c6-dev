...
static bool
inside_canvas(mu_Rect r, int screen_w, int screen_h) {
    return r.x >= 0 && r.y >= 0 && r.w > 0 && r.h > 0 && r.x + r.w <= screen_w && r.y + r.h <= screen_h;
}
...
static void
assert_all_rects_inside(int screen_w, int screen_h) {
...
static void
test_every_rect_inside_canvas_portrait(void) {
    assert_all_rects_inside(PORTRAIT_W, PORTRAIT_H);
}

static void
test_every_rect_inside_canvas_landscape(void) {
...
static void
assert_panels_stacked(int screen_w, int screen_h) {
    const brush_screen_layout_t l = fixture(screen_w, screen_h);

    TEST_ASSERT_FALSE_MESSAGE(rects_overlap(l.header_panel, l.mode_panel), "header and mode panels overlap");
    TEST_ASSERT_FALSE_MESSAGE(rects_overlap(l.mode_panel, l.size_panel), "mode and size panels overlap");
    TEST_ASSERT_FALSE_MESSAGE(rects_overlap(l.header_panel, l.size_panel), "header and size panels overlap");

    TEST_ASSERT_TRUE_MESSAGE(l.header_panel.y + l.header_panel.h <= l.mode_panel.y, "header does not sit above mode");
    TEST_ASSERT_TRUE_MESSAGE(l.mode_panel.y + l.mode_panel.h <= l.size_panel.y, "mode does not sit above size");
}

static void
test_panels_stacked_top_to_bottom_portrait(void) {
    assert_panels_stacked(PORTRAIT_W, PORTRAIT_H);
}

static void
test_panels_stacked_top_to_bottom_landscape(void) {
...
static void
assert_segments_equal_and_contained(int screen_w, int screen_h) {
    const brush_screen_layout_t l = fixture(screen_w, screen_h);

    for (int i = 0; i < BRUSH_SCREEN_SEGMENT_COUNT; i++) {
        TEST_ASSERT_TRUE_MESSAGE(
            inside_canvas(l.segments[i], l.mode_panel.x + l.mode_panel.w, l.mode_panel.y + l.mode_panel.h)
                && l.segments[i].x >= l.mode_panel.x && l.segments[i].y >= l.mode_panel.y,
            "a segment leaves the mode panel");
    }

    TEST_ASSERT_EQUAL_INT_MESSAGE(l.segments[0].w, l.segments[1].w, "segment 0 and 1 widths differ");
    TEST_ASSERT_EQUAL_INT_MESSAGE(l.segments[1].w, l.segments[2].w, "segment 1 and 2 widths differ");
...
static void
test_segments_equal_and_contained_portrait(void) {
    assert_segments_equal_and_contained(PORTRAIT_W, PORTRAIT_H);
}

static void
test_segments_equal_and_contained_landscape(void) {
...
static void
assert_header_shapes(int screen_w, int screen_h) {
    const brush_screen_layout_t l = fixture(screen_w, screen_h);
    ...
    TEST_ASSERT_FALSE_MESSAGE(rects_overlap(l.info_button, l.material_name), "info button overlaps the material name");
}

static void
test_header_shapes_portrait(void) {
    assert_header_shapes(PORTRAIT_W, PORTRAIT_H);
}

static void
test_header_shapes_landscape(void) {
...
static void
assert_size_row(int screen_w, int screen_h) {
    const brush_screen_layout_t l = fixture(screen_w, screen_h);
    ...
    TEST_ASSERT_FALSE_MESSAGE(rects_overlap(l.size_caption, l.size_value), "size caption and value overlap");
}

static void
test_size_row_portrait(void) {
    assert_size_row(PORTRAIT_W, PORTRAIT_H);
}

static void
test_size_row_landscape(void) {
...
static void
assert_tap_targets(int screen_w, int screen_h) {
    const brush_screen_layout_t l = fixture(screen_w, screen_h);
...
static void
test_tap_targets_portrait(void) {
    assert_tap_targets(PORTRAIT_W, PORTRAIT_H);
}

static void
test_tap_targets_landscape(void) {
...
static void
assert_captions_fit(int screen_w, int screen_h) {
    const brush_screen_layout_t l = fixture(screen_w, screen_h);
    ...
    for (unsigned i = 0; i < sizeof fixed / sizeof fixed[0]; i++) {
        TEST_ASSERT_TRUE_MESSAGE(gfx_font_text_width(gfx_font_ui(), fixed[i].str, -1, scale) <= fixed[i].r.w,
                                 fixed[i].str);
    }
    ...
    for (int seg = 0; seg < BRUSH_SCREEN_SEGMENT_COUNT; seg++) {
        ...
        TEST_ASSERT_TRUE_MESSAGE(gfx_font_text_width(gfx_font_ui(), caption, -1, scale) <= l.size_caption.w, caption);

        const char* label = brush_screen_segment_label((brush_screen_segment_t)seg);
        TEST_ASSERT_TRUE_MESSAGE(gfx_font_text_width(gfx_font_ui(), label, -1, scale) <= l.segments[seg].w, label);
    }

    /* The widest the value ever gets: two digits, a space and "PX". */
    TEST_ASSERT_TRUE_MESSAGE(gfx_font_text_width(gfx_font_ui(), "00 PX", -1, scale) <= l.size_value.w,
                             "the size value must fit its own box at every radius");
}

static void
test_captions_fit_their_rects_portrait(void) {
    assert_captions_fit(368, 448);
}

static void
test_captions_fit_their_rects_landscape(void) {

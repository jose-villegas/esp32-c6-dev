...
static bool
magnitude_within(int x, int y, int z, int counts_per_g, int lo_pct, int hi_pct) {
    const int64_t mag2 = (int64_t)x * x + (int64_t)y * y + (int64_t)z * z;
    const int64_t g2 = (int64_t)counts_per_g * counts_per_g;

    return mag2 * 10000 >= g2 * lo_pct * lo_pct && mag2 * 10000 <= g2 * hi_pct * hi_pct;
}

static bool
magnitude_below(int x, int y, int z, int counts_per_g, int pct) {
    const int64_t mag2 = (int64_t)x * x + (int64_t)y * y + (int64_t)z * z;
    const int64_t g2 = (int64_t)counts_per_g * counts_per_g;
...
void
tilt_reset(tilt_t* t, int counts_per_g) {
    t->gx_q8 = 0;
    t->gy_q8 = 0;
    t->shake_q8 = 0;
...
static int32_t
approach(int32_t current_q8, int target, int tau_ms, uint32_t dt_ms) {
...
void
tilt_update(tilt_t* t, int gx, int gy, int gz, int rotation, uint32_t dt_ms) {
     ...
     * hard it is being shaken. */
    {
        ...
        if (step == 0) {
            ...
            t->shake_q8 += (int32_t)(((int64_t)(target - t->shake_q8) * step) / (TILT_SHAKE_TAU_MS + (int)step));
        }
    }
    ...
    t->free_fall = magnitude_below(gx, gy, gz, t->counts_per_g, TILT_FREE_FALL_PCT);
    if (t->free_fall) {
        return;
    }
    ...
    if (!magnitude_within(gx, gy, gz, t->counts_per_g, TILT_TRUST_LO_PCT, TILT_TRUST_HI_PCT)) {
        return;
    }
    ...
    const int tau = TILT_TAU_STILL_MS - ((TILT_TAU_STILL_MS - TILT_TAU_MOVING_MS) * rotation) / 255;
...
int
tilt_x(const tilt_t* t) {
    return t->gx_q8 / Q;
}

int
tilt_y(const tilt_t* t) {
    return t->gy_q8 / Q;
}
...
bool
tilt_in_free_fall(const tilt_t* t) {
    return t->free_fall;
}

...
static void
test_two_separate_active_spots_in_the_same_block_row_do_not_wake_each_other(void) {
    loc_fixture();
    ...
    free(left_before);
    loc_free();
    TEST_ASSERT_TRUE_MESSAGE(left_unchanged, "a settled heap must stay undisturbed while an unrelated stream "
...
static void
test_a_block_wakes_when_disturbed_diagonally(void) {
    loc_fixture();
    ...
    loc_free();
    TEST_ASSERT_EQUAL_UINT8_MESSAGE(SAND_EMPTY, old_cell, "the grain must have left its old cell");
...
test_sideways_tilt_wakes_only_the_disturbed_column(void) {
    ...
    TEST_ASSERT_TRUE_MESSAGE(right_unchanged, "a disturbance far along x must not reach a settled grain two "
...
static void
test_liquid_cross_flow_wakes_only_the_blocks_it_touches_by_range(void) {
...
static void
test_sand_pushing_water_up_wakes_the_dry_row_it_lands_in(void) {
    fixture();
    ...
    TEST_ASSERT_GREATER_THAN_INT_MESSAGE(1, water_cells_in_the_row_above_the_pool,
...
static void
test_water_crosses_a_block_boundary_sideways(void) {
...
static void
test_falling_is_exact_when_scatter_is_off(void) {
    fixture();
...
static void
test_scatter_spreads_a_falling_stream(void) {
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(8, narrow, "without scatter the stream stays exactly one column wide");
    TEST_ASSERT_GREATER_THAN_MESSAGE(narrow, spread, "with scatter it must disperse - that is the whole point");
}
...
static void
test_a_lagging_grain_is_not_left_asleep(void) {

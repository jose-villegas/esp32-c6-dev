...
static void
test_find_reports_two_runs_for_two_separate_blobs(void) {
...
static void
test_find_reports_nothing_for_an_empty_row(void) {
...
static void
test_find_reports_one_run_for_a_fully_occupied_row(void) {
...
static void
test_find_gives_up_past_the_cap(void) {
...
static void
test_span_fallback_reports_empty_range_for_an_empty_row(void) {
...
static bool
all_covered(int x0, int x1, const uint16_t* send_x0, const uint16_t* send_x1, int n) {
    for (int x = x0; x < x1; x++) {
        bool covered = false;
        for (int i = 0; i < n; i++) {
...
static void
test_reconcile_unions_a_current_run_with_the_overlapping_previous_one(void) {
    ...
    const int n = row_runs_reconcile(cur_x0, cur_x1, 1, prev_x0, prev_x1, 1, send_x0, send_x1);
...
static void
test_reconcile_keeps_two_non_overlapping_runs_separate(void) {
    const uint16_t cur_x0[] = {0, 50}, cur_x1[] = {5, 55};
    const uint16_t prev_x0[] = {0, 50}, prev_x1[] = {5, 55};
    uint16_t send_x0[4], send_x1[4];

    const int n = row_runs_reconcile(cur_x0, cur_x1, 2, prev_x0, prev_x1, 2, send_x0, send_x1);
...
static void
test_reconcile_still_sends_a_run_that_vanished_entirely(void) {
    ...
    const int n = row_runs_reconcile(NULL, NULL, 0, prev_x0, prev_x1, 1, send_x0, send_x1);
...
static void
test_reconcile_sends_a_genuinely_new_blob_untouched(void) {
    ...
    const int n = row_runs_reconcile(cur_x0, cur_x1, 1, NULL, NULL, 0, send_x0, send_x1);
...
static void
test_reconcile_handles_a_blob_splitting_into_two(void) {
    ...
    const int n = row_runs_reconcile(cur_x0, cur_x1, 2, prev_x0, prev_x1, 1, send_x0, send_x1);
...
static void
test_reconcile_handles_two_blobs_merging_into_one(void) {
    ...
    const int n = row_runs_reconcile(cur_x0, cur_x1, 1, prev_x0, prev_x1, 2, send_x0, send_x1);
...
static void
test_reconcile_handles_a_new_blob_in_a_previously_empty_gap(void) {
    ...
    TEST_ASSERT_TRUE_MESSAGE(all_covered(0, 10, send_x0, send_x1, n), "the unchanged old run must still be covered");

...
typedef struct sand_s {
    ...
    int crust;                 /* see sand_set_crust(); -1 uses reaction_t.crusts */
...
/* impulse_t and the whole outward-flight API - sand_enable_impulses()
 * through sand_explode() - live in sand_impulse.h, included above, kept
 * together the same way the implementation is kept together in
 * sand_impulse.c. */
...
/* How fast a WATER/ACID impulse's `speed` decays per cell of travel - a
 ...
 * alone. Relay gets a FLOOR, not a ceiling. Transfer fills to everything
 * except SAND_CASCADE_RELAY_RESERVE; relay takes the remainder - a
 * single-mechanism scene is untouched, and relay can never starve below
 * its reserve. */
#define SAND_CASCADE_RELAY_RESERVE         16
#define SAND_CASCADE_TRANSFER_MAX_PER_STEP (SAND_CASCADE_MAX_PER_STEP - SAND_CASCADE_RELAY_RESERVE)
 ...
 * so 118 is half of the 236 left - an even split, with any lean left to
 * the mass bias. */
#define SAND_ACID_DILUTE_TO_WATER_CHANCE   118

/* MASS MATTERS - a fixed-split roll alone can't tell a drop of acid on a
 * lake from a poured slab, so step_one_dissolver_cell() counts each
 * side's cardinal same-material neighbours (0-3) and adjusts
 * SAND_ACID_DILUTE_TO_WATER_CHANCE by the DIFFERENCE, not either count
 * alone - measuring one side lets a deep pool always read "backed". Known
 * asymmetry, not fixed here: density makes acid disperse when poured on
 * water but not the reverse. */
 ...
 * step_one_dissolver_cell() paying only a flat one-unit chip per bite, so
 * a freely dissolvable material let a full acid cell land many bites
 * before running out. Same fix as oil's SAND_ACID_OIL_DEATH_CHANCE,
 * picked lower than oil's 128 because sand/wood/stone dissolve far more
 * readily - oil's rate here would kill acid on nearly every bite. */
#define SAND_ACID_EAT_DEATH_CHANCE         40

/* ACID RAIN - a 2x2 block of exactly two MAT_GAS and two MAT_STEAM cells
 * has a small chance to collapse to one cell, 50/50 Acid or Water (see
 * step_one_acid_rain_cell()) - an extension of the steam-to-water
 * condensation trick. Only the corner survives: acid is a big gas/steam
 * producer, so a full-count yield would keep a lake topped up
 * indefinitely. 1 is already the floor a byte-wide roll allows. */
 ...
 * sight levels faster but leaves a wider band looking momentarily
 * uneven. Flow halts at a non-matching liquid. */
 ...
 * condensation, set chance to 255. Tests needing "no mechanic destroys
 * cells" should also disable acid rain (SAND_ACID_RAIN_CHANCE) if the
 * board supports MAT_GAS and MAT_STEAM. */
void sand_set_condenses(sand_t* s, int chance);
#define SAND_CONDENSES_PER_MATERIAL (-1)

/* A QUENCHING liquid (PAIR_QUENCHES) speeds up a heat-ramping cell's
 * cooling in the ABOVE-ambient branch, per step_one_tempered_cell().
 * Prevents water from dropping below room temperature. The `cools` field
 * already indicates heat-shedding rate, and wet surfaces cool faster. Use
 * one figure for water and acid, the relevant coolants. */
#define SAND_WET_COOLING_FACTOR     8

/* How often a burning LIQUID (lava) that has just done real WORK -
 * converting a neighbour, not merely banking heat - freezes ITSELF as the
 * cost; the same chance cool_off_chain() reaches for at each further pool
 * link. Chance in 256 PER EVENT, not per step, since this only rolls on
 * something already rare. Raising this, not SAND_LAVA_COOLOFF_MAX_CHAIN,
 * keeps pour rate, not chain length, the thing that sets how fast a pool
 * dies. */
...
/* sand_step uses this, not the plain quantisation above: grains move to
 * neighbours, so a slow tilt that always rounds the same way reads as
 * rigid. Dithering the direction over time smooths this - at 17 degrees,
 * 62% of frames fall straight down, 38% down-right, integrating to a
 * smooth flow. Exactly-aligned input is never dithered, so gravity (0, 1)
 * is always straight down and tests stay deterministic. */
void sand_gravity_direction_dithered(sand_t* s, int gx, int gy, int* dx, int* dy);

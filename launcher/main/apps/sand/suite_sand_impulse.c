 ...
 * OUT-OF-RANGE RADII. */
static void
test_the_disc_count_table_matches_a_direct_lattice_count(void) {
    int first_bad = -1;
...
test_a_blast_inside_a_sealed_vessel_stays_inside_it(void) {
    ...
    for (int y = 0; y < H; y++) {
        for (int x = 0; x < W; x++) {
            if (x == 0 || x == W - 1 || y == 0 || y == H - 1) {
                TEST_ASSERT_EQUAL_UINT8_MESSAGE(MAT_STONE, CELL_MATERIAL(sand_at(&s, x, y)),
...
test_a_pane_knocked_loose_is_queued_as_cullet(void) {
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(1, s.impulse_count, "the dislodge has to have queued the pane in the first place");
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(SAND_EXPLODE_INITIAL_SPEED, s.impulse_buf[0].speed,
                                  "and must keep the speed it was given");
...
test_a_shattered_pane_flies_the_way_it_was_pushed(void) {
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(0, count_cells_of(MAT_GLASS), "no pane may survive the throw");
    TEST_ASSERT_TRUE_MESSAGE(landed > 1, "the shard has to have travelled east, the way it was pushed - "
...
static void
test_a_blast_at_the_edge_stays_in_bounds(void) {
...
static void
test_the_cap_degrades_gracefully(void) {
    fixture();
    ...
    TEST_ASSERT_EQUAL_UINT16_MESSAGE((uint16_t)(7 * W + 3), s.impulse_buf[0].index,
    ...
    TEST_ASSERT_EQUAL_UINT16_MESSAGE((uint16_t)(6 * W + 4), s.impulse_buf[1].index,
    ...
    TEST_ASSERT_NOT_EQUAL_MESSAGE(SAND_EMPTY, sand_at(&s, 2, 6), "LEFT did not fit either, for the same reason");
...
test_two_overlapping_blasts_share_the_buffer_evenly(void) {
    ...
    TEST_ASSERT_EQUAL_UINT16_MESSAGE((uint16_t)(1 * W + 5), s.impulse_buf[4].index,
                                     "worked by hand against room=4 (see this test's own top comment): "
                                     "UP, (5,1), fires first");
    TEST_ASSERT_EQUAL_UINT16_MESSAGE((uint16_t)(3 * W + 6), s.impulse_buf[5].index, "down-right, (6,3), fires second");
    TEST_ASSERT_EQUAL_UINT16_MESSAGE((uint16_t)(0 * W + 5), s.impulse_buf[6].index,
    ...
    TEST_ASSERT_EQUAL_UINT16_MESSAGE((uint16_t)(2 * W + 7), s.impulse_buf[7].index,
...
static void
test_a_blast_wakes_the_blocks_it_touches(void) {
 ...
 * mechanism. */
static void
test_a_flying_grain_keeps_its_outward_push_while_falling(void) {
...
static void
test_a_spent_static_chunk_rests_on_a_powder_bank_instead_of_sinking_forever(void) {
    fixture();
    sand_clear(&s);
    sand_enable_impulses(&s, impulse_buf, W * H);
...
static void
test_a_static_chunk_thrown_far_still_stops_shallow_in_the_bed_it_hits(void) {
    uint8_t* cells = malloc((size_t)FAR_SINK_W * FAR_SINK_H);
    TEST_ASSERT_NOT_NULL_MESSAGE(cells, "far-sink grid must fit in what the framebuffer leaves");
    impulse_t* buf = malloc((size_t)(FAR_SINK_W * FAR_SINK_H) * sizeof *buf);
    TEST_ASSERT_NOT_NULL_MESSAGE(buf, "far-sink impulse queue must fit in what the framebuffer leaves");
...
static void
test_a_spent_static_chunk_still_sinks_through_water_to_the_bottom(void) {
    fixture();
    sand_clear(&s);
    sand_enable_impulses(&s, impulse_buf, W * H);
...
test_a_thrown_static_chunk_conserves_lava_mass_on_sink(void) {
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(lava_mass_before, mass_of(&s, W, H, MAT_LAVA),
...
test_a_chunk_stacked_on_an_in_flight_chunk_waits_instead_of_settling_and_both_eventually_land(void) {
    ...
    TEST_ASSERT_EQUAL_UINT8_MESSAGE(MAT_STONE, CELL_MATERIAL(sand_at(&s, COL, TOP_ROW)),
    ...
    TEST_ASSERT_NOT_EQUAL_MESSAGE(MAT_STONE, CELL_MATERIAL(sand_at(&s, COL, TOP_ROW)),
...
ordinary_static_solid_scene(bool flip) {
    ...
    for (int i = 0; i < H; i++) {
        ...
        for (int q = 0; q < s.impulse_count; q++) {
            TEST_ASSERT_NOT_EQUAL_MESSAGE(KIND_STATIC, material_of(s.impulse_buf[q].cell)->kind,
    ...
    TEST_ASSERT_EQUAL_UINT8_MESSAGE(MAT_STONE, CELL_MATERIAL(sand_at(&s, 2, 0)),
    ...
    TEST_ASSERT_EQUAL_UINT8_MESSAGE(MAT_STONE, CELL_MATERIAL(sand_at(&s, 5, 0)),
                                    "same for a powder - an ordinary KIND_STATIC cell must still just "
                                    "sit exactly where it was placed");
}

static void
test_an_ordinary_static_solid_still_does_not_sink_into_liquid_or_powder(void) {
 ...
 * claims. */
static void
plow_build(sand_t* g, uint8_t* cells, impulse_t* buf, int buf_max, uint32_t seed, material_id_t mover, cell_t medium) {
...
static long
plow_total_distance(uint8_t* cells, impulse_t* buf, cell_t medium) {
    long total = 0;
...
static void
test_a_thrown_chunk_travels_less_far_through_dirt_than_through_air(void) {
    uint8_t* cells = malloc((size_t)PLOW_W * PLOW_H);
    TEST_ASSERT_NOT_NULL_MESSAGE(cells, "plow grid must fit in what the framebuffer leaves");
    impulse_t buf[4];
...
static void
test_a_thrown_chunk_stops_near_the_rim_of_a_dirt_bank(void) {
    uint8_t* cells = malloc((size_t)PLOW_W * PLOW_H);
    TEST_ASSERT_NOT_NULL_MESSAGE(cells, "plow grid must fit in what the framebuffer leaves");
    impulse_t buf[4];
...
static void
test_a_thrown_chunk_travels_less_far_through_dirt_than_through_water(void) {
    uint8_t* cells = malloc((size_t)PLOW_W * PLOW_H);
    TEST_ASSERT_NOT_NULL_MESSAGE(cells, "plow grid must fit in what the framebuffer leaves");
    impulse_t buf[4];
...
test_a_thrown_liquid_grain_pays_no_drag_displacing_water(void) {
    ...
    const int extra_cells = (int)expect_speed / SAND_IMPULSE_CELLS_PER_STEP_DIVISOR;
    for (int c = 0; c < extra_cells; c++) {
        expect_speed -= expect_speed >> SAND_SPLASH_SPEED_DECAY_SHIFT;
    }
    TEST_ASSERT_EQUAL_INT_MESSAGE((int)expect_speed, s.impulse_buf[0].speed,
...
test_a_thrown_powder_grain_bounces_off_a_wall_instead_of_waiting(void) {
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(DIR_LEFT, s.impulse_buf[0].dir,
                                  ...
                                  "of its flight");
    TEST_ASSERT_EQUAL_INT_MESSAGE((255 - SAND_IMPULSE_SPEED_RAMP) >> 1, s.impulse_buf[0].speed,
...
test_a_thrown_chunk_displacing_nothing_loses_only_the_plain_ramp(void) {
    ...
    const int air_cells = 1 + (255 - SAND_IMPULSE_SPEED_RAMP) / SAND_IMPULSE_CELLS_PER_STEP_DIVISOR;
    TEST_ASSERT_EQUAL_INT_MESSAGE(255 - SAND_IMPULSE_SPEED_RAMP * air_cells, s.impulse_buf[0].speed,
 ...
 * decay. */
static void
test_a_sub_divisor_speed_impulse_never_moves_more_than_one_cell_a_step(void) {
    ...
    for (int i = 0; i < SUBDIV_STEPS; i++) {
        ...
        const int x = (int)((unsigned)g.impulse_buf[0].index % (unsigned)SUBDIV_W);
...
test_a_full_speed_static_chunk_moves_several_cells_in_one_push(void) {
    ...
    const int expected_cells = 1 + (255 - SAND_IMPULSE_SPEED_RAMP) / SAND_IMPULSE_CELLS_PER_STEP_DIVISOR;
...
static void
test_blocker_normal_and_reflect_off_normal_match_the_exhaustive_arc_table(void) {
    fixture();
    sand_clear(&s);
    ...
    for (int dir = 0; dir < 8; dir++) {
        ...
        for (int cfg = 0; cfg < 4; cfg++) {
            ...
            snprintf(msg, sizeof msg,
                     ...
                     "match the ground-truth table exactly",
                     dir, cfg, has_l, has_r);
...
test_a_chunk_bounces_off_the_grid_edge_instead_of_waiting_there_forever(void) {
    ...
    for (uint32_t k = 1; k <= (uint32_t)EDGE_SEEDS; k++) {
        ...
        snprintf(msg, sizeof msg,
                 ...
                 "and the plain wait never changes direction",
                 (unsigned)k, EDGE_X);
...
static void
test_a_blast_queues_impulses_on_every_side_of_the_centre(void) {
    fixture();
    sand_enable_impulses(&s, axis_impulse_buf, 8);
    ...
    TEST_ASSERT_TRUE_MESSAGE(saw_up, "an impulse above the centre must be queued - this direction "
                                     "worked even under the old bug, so its absence here would mean "
                                     "something else broke");
    TEST_ASSERT_TRUE_MESSAGE(saw_down, "an impulse below the centre must be queued - this is exactly "
                                       "the direction the old top-to-bottom scan order starved first, "
                                       "along with the entire core between it and the centre");
    TEST_ASSERT_TRUE_MESSAGE(saw_left, "an impulse left of the centre must be queued");
    TEST_ASSERT_TRUE_MESSAGE(saw_right, "an impulse right of the centre must be queued");
}
    ...
    for (int y = 0; y < H; y++) {
        for (int x = 0; x < W; x++) {
            ...
            if (dx * dx + dy * dy <= core_r2) {
                TEST_ASSERT_EQUAL_UINT8_MESSAGE(MAT_FIRE, CELL_MATERIAL(sand_at(&s, x, y)),
...
static void
test_without_a_buffer_explode_does_nothing(void) {
    fixture();
...
static void
test_the_two_wall_explosion_scene_bounces_more_than_once_before_settling(void) {
    uint8_t* ricochet_cells = malloc((size_t)RICOCHET_W * RICOCHET_H);
    TEST_ASSERT_NOT_NULL_MESSAGE(ricochet_cells, "ricochet grid must fit in what the framebuffer leaves");
    impulse_t* ricochet_buf = malloc((size_t)(RICOCHET_W * RICOCHET_H) * sizeof *ricochet_buf);
    TEST_ASSERT_NOT_NULL_MESSAGE(ricochet_buf, "ricochet impulse queue must fit in what the framebuffer leaves");
    ricochet_tracker_t* ricochet_tracker = malloc(sizeof *ricochet_tracker);
    TEST_ASSERT_NOT_NULL_MESSAGE(ricochet_tracker, "ricochet tracker must fit in what the framebuffer leaves");
    ...
    for (uint32_t seed = 1; seed <= (uint32_t)RICOCHET_SEEDS; seed++) {
        ricochet_measure_seed(ricochet_cells, ricochet_buf, ricochet_tracker, seed, hist, &total_entries,
                              &any_unmatched_new);
    }
    ...
    TEST_ASSERT_FALSE_MESSAGE(any_unmatched_new, "a tracked entry appeared with no plausible predecessor within "
 ...
 * same thing in both. */
static int
ejecta_count_outside(sand_t* g, int gw, int gh, uint8_t mat, int x0, int x1, int y0, int y1) {
    int n = 0;
    for (int y = 0; y < gh; y++) {
        for (int x = 0; x < gw; x++) {
...
static int
ejecta_count_material(sand_t* g, int gw, int gh, uint8_t mat) {
    int n = 0;
    for (int y = 0; y < gh; y++) {
        for (int x = 0; x < gw; x++) {
            if (CELL_MATERIAL(sand_at(g, x, y)) == mat) {
...
static long
ejecta_dirt_total(uint8_t* cells, impulse_t* buf) {
    long total = 0;
...
static void
test_a_thrown_powder_grain_flings_dirt_out_of_the_bank_it_hits(void) {
    uint8_t* cells = malloc((size_t)EJECTA_W * EJECTA_H);
    TEST_ASSERT_NOT_NULL_MESSAGE(cells, "ejecta grid must fit in what the framebuffer leaves");
    impulse_t* buf = malloc((size_t)(EJECTA_W * EJECTA_H) * sizeof *buf);
    TEST_ASSERT_NOT_NULL_MESSAGE(buf, "ejecta impulse queue must fit in what the framebuffer leaves");
...
static int
airborne_bed_peak(uint8_t* cells, impulse_t* buf, uint32_t seed) {
    sand_t g;
    ...
    for (int k = 0; k < AIRBORNE_CHUNK_W; k++) {
        ...
        sand_impulse_dislodge(&g, AIRBORNE_CHUNK_X0 + k, AIRBORNE_CHUNK_Y, DIR_DOWN_RIGHT, 255,
...
static void
test_a_stone_chunk_thrown_into_a_sand_bed_launches_sand_airborne(void) {
    uint8_t* cells = malloc((size_t)AIRBORNE_W * AIRBORNE_H);
    TEST_ASSERT_NOT_NULL_MESSAGE(cells, "airborne-bed grid must fit in what the framebuffer leaves");
    impulse_t* buf = malloc((size_t)(AIRBORNE_W * AIRBORNE_H) * sizeof *buf);
    TEST_ASSERT_NOT_NULL_MESSAGE(buf, "airborne-bed impulse queue must fit in what the framebuffer leaves");
...
static void
test_a_powder_grain_thrown_far_still_ejects_from_the_bank_it_hits(void) {
    uint8_t* cells = malloc((size_t)EJECTA_FAR_W * EJECTA_FAR_H);
    TEST_ASSERT_NOT_NULL_MESSAGE(cells, "far-throw ejecta grid must fit in what the framebuffer leaves");
    impulse_t* buf = malloc((size_t)(EJECTA_FAR_W * EJECTA_FAR_H) * sizeof *buf);
    TEST_ASSERT_NOT_NULL_MESSAGE(buf, "far-throw ejecta impulse queue must fit in what the framebuffer "
                                      "leaves");
...
static long
ejecta_water_total(uint8_t* cells, impulse_t* buf) {
    long total = 0;
    for (uint32_t seed = 1; seed <= (uint32_t)EJECTA_SEEDS; seed++) {
...
static void
test_a_thrown_powder_grain_flings_water_out_of_the_pool_it_hits(void) {
    uint8_t* cells = malloc((size_t)EJECTA_W * EJECTA_H);
    TEST_ASSERT_NOT_NULL_MESSAGE(cells, "ejecta grid must fit in what the framebuffer leaves");
    impulse_t* buf = malloc((size_t)(EJECTA_W * EJECTA_H) * sizeof *buf);
    TEST_ASSERT_NOT_NULL_MESSAGE(buf, "ejecta impulse queue must fit in what the framebuffer leaves");
...
test_a_struck_water_cell_is_handed_impulse_in_a_backward_cone_from_the_mover(void) {
    ...
    const uint8_t mover_speed =
        (uint8_t)(255 - SAND_IMPULSE_SPEED_RAMP - impulse_drag_of(CELL_MAKE(MAT_WATER, MASS_MAX)));
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(MAT_WATER, CELL_MATERIAL(s.impulse_buf[found].cell),
                                  "and the entry queued there must be the WATER that was struck, not "
                                  "the chunk that struck it");

    const int cone[3] = {(DIR_RIGHT + 3) & 7, (DIR_RIGHT + 4) & 7, (DIR_RIGHT + 5) & 7};
    bool in_cone = false;
    for (int k = 0; k < 3; k++) {
    ...
    TEST_ASSERT_TRUE_MESSAGE(in_cone, "the struck cell's direction must be one of dir+3/dir+4/dir+5 - "
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE((int)(((unsigned)mover_speed * SAND_IMPULSE_TRANSFER_KEEP) >> 8),
...
static long
plow_total_distance_powder(uint8_t* cells, impulse_t* buf, cell_t medium) {
    long total = 0;
    for (uint32_t k = 1; k <= (uint32_t)PLOW_SEEDS; k++) {
        sand_t g;
...
static void
test_a_thrown_powder_grain_travels_less_far_through_dirt_than_through_air(void) {
    uint8_t* cells = malloc((size_t)PLOW_W * PLOW_H);
    TEST_ASSERT_NOT_NULL_MESSAGE(cells, "plow grid must fit in what the framebuffer leaves");
    impulse_t buf[4];

    const long air_total = plow_total_distance_powder(cells, buf, 0);
    const long dirt_total = plow_total_distance_powder(cells, buf, CELL_MAKE(MAT_DIRT, 0));
...
static void
test_a_long_plow_through_a_wide_bank_never_exhausts_the_impulse_buffer(void) {
    uint8_t* cells = malloc((size_t)BUDGET_W * BUDGET_H);
    TEST_ASSERT_NOT_NULL_MESSAGE(cells, "budget-guard grid must fit in what the framebuffer leaves");
    impulse_t* buf = malloc((size_t)BUDGET_IMPULSE_MAX * sizeof *buf);
    TEST_ASSERT_NOT_NULL_MESSAGE(buf, "budget-guard impulse queue must fit in what the framebuffer "
                                      "leaves");
    ...
    for (int i = 0; i < BUDGET_STEPS; i++) {
        ...
        snprintf(why, sizeof why,
                 ...
                 "did not hold",
                 i, g.impulse_count, BUDGET_IMPULSE_MAX);
        TEST_ASSERT_LESS_OR_EQUAL_INT_MESSAGE(BUDGET_IMPULSE_MAX, g.impulse_count, why);
    }
...
test_a_thrown_powder_grain_conserves_the_dirt_it_ejects(void) {
    uint8_t* cells = malloc((size_t)EJECTA_W * EJECTA_H);
    TEST_ASSERT_NOT_NULL_MESSAGE(cells, "conservation grid must fit in what the framebuffer leaves");
    impulse_t* buf = malloc((size_t)(EJECTA_W * EJECTA_H) * sizeof *buf);
    TEST_ASSERT_NOT_NULL_MESSAGE(buf, "conservation impulse queue must fit in what the framebuffer "
                                      "leaves");
...
static void
test_a_thrown_powder_grain_conserves_the_water_mass_it_ejects(void) {
    uint8_t* cells = malloc((size_t)EJECTA_W * EJECTA_H);
    TEST_ASSERT_NOT_NULL_MESSAGE(cells, "conservation grid must fit in what the framebuffer leaves");
    impulse_t* buf = malloc((size_t)(EJECTA_W * EJECTA_H) * sizeof *buf);
    TEST_ASSERT_NOT_NULL_MESSAGE(buf, "conservation impulse queue must fit in what the framebuffer "
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE((int)(water_mass_before - MASS_MAX), (int)water_mass_after,
...
test_nothing_moves_in_free_fall(void) {
    ...
    TEST_ASSERT_NOT_EQUAL_MESSAGE(SAND_EMPTY, sand_at(&s, 3, 0), "with no gravity there is no down, so nothing falls");
}

static void
test_shaking_spreads_a_pile_sideways(void) {
    fixture();

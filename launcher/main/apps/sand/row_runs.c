...
int
row_runs_find(const uint8_t* row, int width, uint8_t empty, int* run_x0, int* run_x1) {
    int n = 0;
    int cx = 0;
...
void
row_runs_span_fallback(const uint8_t* row, int width, uint8_t empty, int* x0, int* x1) {
    int min_cx = width;
    int max_cx = -1;
...
static void
absorb_overlapping(uint16_t* x0, uint16_t* x1, const uint16_t* prev_x0, const uint16_t* prev_x1, int prev_n,
                   bool* prev_used) {
...
int
row_runs_reconcile(const uint16_t* cur_x0, const uint16_t* cur_x1, int cur_n, const uint16_t* prev_x0,

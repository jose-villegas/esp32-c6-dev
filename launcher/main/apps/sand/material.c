...
/* See material_set_foam_phase() in material.h. Zero until first frame sets
 * it. */
...
/* Cullet phase: see material_set_cullet_phase() in material.h. Starts at
...
material_colours(cell_t c, unsigned hash, unsigned mask, unsigned depth, gfx_color_t out[3]) {
    ...
    if (material_of(c)->kind == KIND_LIQUID) {
        ...
        if (cardinal == 0) {
             ...
             * MATERIAL_EDGE_CARDINAL in material.h. */
    ...
    switch (CELL_MATERIAL(c)) {
        ...
        case MAT_STONE:
            /* See MATERIAL_EDGE_CARDINAL's comment in material.h. */
            out[0] =

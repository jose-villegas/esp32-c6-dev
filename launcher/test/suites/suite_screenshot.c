...
static void
test_row_stride_pads_up_to_next_multiple_of_four(void) {
...
static void
test_header_starts_with_bm_magic(void) {
...
static void
test_header_file_size_covers_header_plus_padded_pixels(void) {
    ...
    const uint32_t file_size =
        (uint32_t)header[2] | ((uint32_t)header[3] << 8) | ((uint32_t)header[4] << 16) | ((uint32_t)header[5] << 24);
    ...
    const uint32_t off_bits = (uint32_t)header[10] | ((uint32_t)header[11] << 8) | ((uint32_t)header[12] << 16)
                              | ((uint32_t)header[13] << 24);
    TEST_ASSERT_EQUAL_UINT32(SCREENSHOT_BMP_HEADER_SIZE, off_bits);
}

static void
test_header_dimensions_and_bit_depth(void) {
    uint8_t header[SCREENSHOT_BMP_HEADER_SIZE];
    screenshot_bmp_header(header, 368, 448);

    const uint32_t width = (uint32_t)header[18] | ((uint32_t)header[19] << 8) | ((uint32_t)header[20] << 16)
...
static void
test_encoded_len_matches_rfc4648_examples(void) {
...
static void
test_encode_one_byte_pads_with_two_equals(void) {
    char out[5] = {0};
...
static void
test_encode_two_bytes_pads_with_one_equals(void) {
    char out[5] = {0};
...
static void
test_encode_three_bytes_needs_no_padding(void) {
    char out[5] = {0};
...
static void
test_encode_a_multi_group_string(void) {

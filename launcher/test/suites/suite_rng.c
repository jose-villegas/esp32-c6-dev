...
static void
test_different_seeds_diverge(void) {
    rng_t a, b;
    rng_seed(&a, 1u);
    rng_seed(&b, 2u);
...
static void
test_a_zero_seed_does_not_stick(void) {
    ...
    TEST_ASSERT_NOT_EQUAL_MESSAGE(0u, combined, "a zero seed must be substituted, not accepted");
}

static void
test_the_state_never_reaches_zero(void) {
    ...
    for (int i = 0; i < 100000; i++) {
        (void)rng_next(&r);
        TEST_ASSERT_NOT_EQUAL_MESSAGE(0u, r.state, "the generator must not be able to fall into its fixed point");
    }
}

static void
test_below_stays_inside_its_bound(void) {
    rng_t r;
    rng_seed(&r, 42u);

    for (int i = 0; i < 5000; i++) {
        const int v = rng_below(&r, 6);
        TEST_ASSERT_TRUE_MESSAGE(v >= 0 && v < 6, "rng_below must never return its own bound - an off-by-one here "
                                                  "indexes one past a palette");
    }
}

static void
test_below_reaches_every_value(void) {
    rng_t r;
    rng_seed(&r, 7u);
    ...
    for (int i = 0; i < 6; i++) {
        TEST_ASSERT_TRUE_MESSAGE(seen[i], "every value in range must actually come up, or a pile ends up "
                                          "using half its shades");
    }
}

static void
test_below_handles_a_useless_bound(void) {
    rng_t r;
    rng_seed(&r, 3u);

    TEST_ASSERT_EQUAL_INT(0, rng_below(&r, 0));
    TEST_ASSERT_EQUAL_INT_MESSAGE(0, rng_below(&r, -5), "a bound of zero or less must be answered, not divided by");
}

static void
test_never_and_always_are_exact(void) {
    rng_t r;
    rng_seed(&r, 99u);

    for (int i = 0; i < 2000; i++) {
        TEST_ASSERT_FALSE_MESSAGE(rng_chance(&r, 0), "a chance of zero must be impossible, not merely unlikely");
        TEST_ASSERT_TRUE_MESSAGE(rng_chance(&r, 256), "and a chance of 256 must be certain - written naively as "
...
static void
test_a_chance_lands_near_its_stated_odds(void) {
    rng_t r;
    rng_seed(&r, 2024u);

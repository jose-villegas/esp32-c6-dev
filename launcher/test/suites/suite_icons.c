...
static int blocks_at(int w, int h, icon_rect_t *out)
{
    ...
    icon_walk_blocks(icon_system_rows + icon->offset, icon->w, icon->h,
                     icon->stride, w, h, collect_emit, &cc);
    return cc.count;
}
...
static void test_check_matches_the_artwork_it_replaced(void)
{
    const icon_t *icon = &icon_system_table[ICON_SYSTEM_CHECK];
...
static int popcount_of(const char *const *rows)
{
    int set = 0;
    for (int y = 0; y < 16; y++) {
        for (int x = 0; x < 16; x++) {
 ...
 * picked for headroom. */
static void test_blocks_matches_the_artworks_actual_run_count(void)
{
    int total_runs = 0;
    for (int y = 0; y < 16; y++) {
        const char *row = check_expected_rows[y];
...
static void test_stroke_is_at_least_2px_native_except_the_one_tapered_tip(void)
{
    ...
    TEST_ASSERT_TRUE_MESSAGE(thin_runs <= 1,
        "more than one run in the artwork is thinner than 2px - the stroke "

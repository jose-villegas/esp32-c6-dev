...
static void
test_a_held_press_is_reported_once(void) {
    fixture();
    advance(0, false);
...
static void
test_an_edge_is_consumed_by_whoever_reads_it(void) {
    fixture();
    advance(0, false);
    hold(40, true);
...
static void
test_contact_bounce_produces_one_press_not_several(void) {
    fixture();
    advance(0, false);
    ...
    hold(40, true);
    TEST_ASSERT_TRUE_MESSAGE(button_fsm_take_pressed(&b), "and exactly one press once it settles");
    TEST_ASSERT_FALSE(button_fsm_take_pressed(&b));
}

static void
test_a_spike_shorter_than_the_debounce_is_ignored(void) {
    fixture();
    advance(0, false);
    ...
    TEST_ASSERT_FALSE_MESSAGE(button_fsm_take_pressed(&b), "a spike far shorter than a human press must not register");
    TEST_ASSERT_FALSE(button_fsm_is_down(&b));
}

static void
test_release_is_debounced_too(void) {
    fixture();
    advance(0, false);
    hold(40, true);
    ...
    advance(2, true);
    advance(2, false);
    TEST_ASSERT_FALSE_MESSAGE(button_fsm_take_released(&b), "a bouncing break must not report a release yet");

    hold(40, false);
    TEST_ASSERT_TRUE_MESSAGE(button_fsm_take_released(&b), "and exactly one release once it settles");
    TEST_ASSERT_FALSE(button_fsm_is_down(&b));
}
...
static void
test_a_full_tap_gives_one_press_and_one_release(void) {
    fixture();
    advance(0, false);
...
static void
test_a_short_press_gives_pressed_then_released_and_never_held(void) {
    fixture();
    advance(0, false);
    ...
    TEST_ASSERT_FALSE_MESSAGE(button_fsm_take_held(&b), "a press this short must never fire held");
}

static void
test_a_long_press_fires_held_once_with_no_release(void) {
    fixture();
    advance(0, false);
    ...
    hold(600, true);
    TEST_ASSERT_TRUE_MESSAGE(button_fsm_take_held(&b), "a press held past BUTTON_HOLD_US must fire held");
...
static void
test_held_does_not_fire_twice_while_still_down(void) {
    fixture();
    advance(0, false);
...
static void
test_hold_is_timed_from_the_debounced_press_not_the_first_raw_sample(void) {
    fixture();
    advance(0, false);
    ...
    advance(500, true);
    TEST_ASSERT_FALSE_MESSAGE(button_fsm_take_held(&b), "the hold must not fire before a full BUTTON_HOLD_US after the "
                                                        "debounced press, not the first raw sample");

    advance(150, true);
    TEST_ASSERT_TRUE_MESSAGE(button_fsm_take_held(&b), "and must fire once a full BUTTON_HOLD_US has elapsed from the "
                                                       "debounced press");
}
...
static void
test_take_held_consumes_the_edge(void) {
    fixture();
    advance(0, false);
    hold(40, true);
    (void)button_fsm_take_pressed(&b);

...
static void
test_round_trip_holds_for_every_value_in_range(void) {
...
static void
test_round_trip_holds_at_a_coarser_step(void) {
    const slider_fixture_t f = fixture();
...
static void
test_a_touch_picks_the_value_whose_knob_centres_nearest_it(void) {
    const slider_fixture_t f = fixture();
    ...
    for (int x = f.track.x - 20; x <= f.track.x + f.track.w + 20; x++) {
        ...
        for (int u = f.lo; u <= f.hi; u++) {
            ...
            TEST_ASSERT_TRUE_MESSAGE(dv <= du, "the knob must land under the finger, not half its width to "
                                               "one side of it");
        }
    }
}
...
static void
test_knob_sits_flush_inside_the_track_at_lo_and_hi(void) {
    const slider_fixture_t f = fixture();
...
static void
test_knob_stays_inside_the_track_everywhere_between(void) {
    const slider_fixture_t f = fixture();

    for (int v = f.lo; v <= f.hi; v++) {
        const mu_Rect k = ui_slider_knob_rect(f.track, f.lo, f.hi, v, f.knob_w);
        TEST_ASSERT_TRUE_MESSAGE(k.x >= f.track.x, "the knob must never start left of the track");
...
static void
test_a_touch_quantizes_to_the_nearest_step(void) {
    const slider_fixture_t f = fixture();
...
static void
test_a_touch_beyond_either_end_clamps_not_wraps(void) {
    const slider_fixture_t f = fixture();

    TEST_ASSERT_EQUAL_INT_MESSAGE(f.lo, ui_slider_value_at_x(f.track, f.lo, f.hi, f.knob_w, f.step, -5000),
                                  "a touch far to the left must clamp to lo, not wrap around");
    TEST_ASSERT_EQUAL_INT_MESSAGE(f.hi, ui_slider_value_at_x(f.track, f.lo, f.hi, f.knob_w, f.step, 5000),
...
static void
test_fill_width_never_exceeds_the_track(void) {
    const slider_fixture_t f = fixture();
...
static void
test_fill_width_grows_with_value(void) {
    const slider_fixture_t f = fixture();
...
static void
test_lo_equals_hi_returns_lo_without_dividing_by_zero(void) {
    const slider_fixture_t f = fixture();

    const mu_Rect k = ui_slider_knob_rect(f.track, 5, 5, 5, f.knob_w);
    TEST_ASSERT_EQUAL_INT(f.track.x, k.x);
    TEST_ASSERT_EQUAL_INT_MESSAGE(5, ui_slider_value_at_x(f.track, 5, 5, f.knob_w, f.step, f.track.x + 137),
                                  "with no range at all, every touch must resolve to the one value there is");
}

static void
test_a_track_narrower_than_the_knob_shrinks_the_knob_to_fit(void) {
    const mu_Rect narrow = {0, 0, 10, 24};

    const mu_Rect k = ui_slider_knob_rect(narrow, 0, 100, 50, 40);
    TEST_ASSERT_TRUE_MESSAGE(k.w <= narrow.w, "a knob must never be drawn wider than the track it lives on");
}
...
static void
test_step_larger_than_the_whole_range_still_clamps(void) {
    const slider_fixture_t f = fixture();

    const int v = ui_slider_value_at_x(f.track, 0, 10, f.knob_w, 1000, f.track.x + f.track.w);
    TEST_ASSERT_TRUE_MESSAGE(v == 0 || v == 10, "a step bigger than the whole range leaves only its two ends "
                                                "reachable");
}

static void
test_zero_width_track_produces_no_oversized_or_negative_rect(void) {

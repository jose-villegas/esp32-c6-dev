...
static void
test_t_255_returns_b_exactly(void) {
    const gfx_color_t a = GFX_RGB(0x336699);
    const gfx_color_t b = GFX_RGB(0x8899AA);
...
test_black_toward_white_at_half_gives_mid_grey(void) {
    ...
    TEST_ASSERT_EQUAL_HEX16_MESSAGE(GFX_RGB(0x808080), gfx_color_mix(black, white, 128),
...
static void
test_a_known_pair_blends_to_a_known_result(void) {
    ...
    TEST_ASSERT_EQUAL_HEX16_MESSAGE(GFX_RGB(0x780080), gfx_color_mix(red, blue, 128),
...
test_channels_blend_independently_green_stays_put(void) {
    ...
    TEST_ASSERT_EQUAL_HEX16_MESSAGE(GFX_RGB(0xB80040), gfx_color_mix(red, blue, 64),
                                    "green must stay at zero while red and blue move independently");
    TEST_ASSERT_EQUAL_HEX16_MESSAGE(GFX_RGB(0x4000B8), gfx_color_mix(red, blue, 192),
                                    "green must stay at zero while red and blue move independently");
}
...
static void
test_adding_two_primaries_gives_their_combination(void) {
    TEST_ASSERT_EQUAL_HEX16_MESSAGE(GFX_RGB(0xFFFF00), gfx_color_add(GFX_RGB(0xFF0000), GFX_RGB(0x00FF00)),
                                    "red plus green is yellow - this is light, not paint");
    TEST_ASSERT_EQUAL_HEX16(GFX_RGB(0xFFFFFF), gfx_color_add(GFX_RGB(0xFFFF00), GFX_RGB(0x0000FF)));
}
...
static void
test_each_channel_saturates_at_its_own_ceiling(void) {
    const gfx_color_t white = GFX_RGB(0xFFFFFF);

    TEST_ASSERT_EQUAL_HEX16_MESSAGE(white, gfx_color_add(white, white),
                                    "white plus white is white, not a wrapped-round mess");
    TEST_ASSERT_EQUAL_HEX16(GFX_RGB(0xFF0000), gfx_color_add(GFX_RGB(0xFF0000), GFX_RGB(0xFF0000)));
    TEST_ASSERT_EQUAL_HEX16(GFX_RGB(0x00FF00), gfx_color_add(GFX_RGB(0x00FF00), GFX_RGB(0x00FF00)));
    TEST_ASSERT_EQUAL_HEX16(GFX_RGB(0x0000FF), gfx_color_add(GFX_RGB(0x0000FF), GFX_RGB(0x0000FF)));
}

static void
test_adding_never_makes_a_channel_darker(void) {
    ...
    for (uint32_t rgb = 0; rgb <= 0xFFFFFFu; rgb += 0x0103F7u) {
        ...
        TEST_ASSERT_TRUE_MESSAGE((ns & 0x1Fu) >= (nb & 0x1Fu), "adding light made the blue channel darker");
    }
}
...
static const uint32_t rgb888_spread[] = {
    0x000000, 0xFFFFFF, 0xF80000, 0x00FC00, 0x0000F8, 0x336699, 0x8899AA, 0x5C7AAF, 0x1A2B3C, 0xE0D0C0,
};
#define RGB888_SPREAD_N (sizeof(rgb888_spread) / sizeof(rgb888_spread[0]))

static void
test_round_trip_is_exact_for_a_spread_of_colours(void) {
...
static void
test_dither_level_agrees_with_covers_at_every_alpha_and_cell(void) {
    for (int a = 0; a <= 255; a++) {
        const int level = gfx_dither_level((uint8_t)a);
        for (int y = 0; y < 4; y++) {
            for (int x = 0; x < 4; x++) {
                TEST_ASSERT_EQUAL_MESSAGE(level > gfx_dither4x4[y][x], gfx_dither_covers(x, y, (uint8_t)a),
...
static void
test_alpha_zero_never_covers(void) {
    for (int y = 0; y < 4; y++) {
        for (int x = 0; x < 4; x++) {
...
static void
test_alpha_255_always_covers(void) {
    for (int y = 0; y < 4; y++) {
        for (int x = 0; x < 4; x++) {
...
static void
test_coverage_is_monotonic_in_alpha_at_every_cell(void) {
    for (int y = 0; y < 4; y++) {
        for (int x = 0; x < 4; x++) {
            ...
            for (int a = 0; a <= 255; a++) {
                ...
                if (was_covered) {
                    TEST_ASSERT_TRUE_MESSAGE(covered, "coverage at a fixed cell must never go from "
...
test_mixing_toward_black_twice_is_darker_than_once(void) {
    ...
    TEST_ASSERT_TRUE_MESSAGE(rgb_twice < rgb_once, "a scrim applied twice must be visibly darker than once - this is "

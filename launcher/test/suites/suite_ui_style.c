...
static int
spans_for(mu_Rect r, bool sunken, ui_span_t* out) {
...
static void
test_shading_leaves_alpha_alone(void) {
...
test_every_span_stays_inside_the_control(void) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        TEST_ASSERT_TRUE_MESSAGE(s[i].rect.x + s[i].rect.w <= ROW_X + ROW_W
...
test_a_raised_button_is_lit_from_the_top_left(void) {
    ...
    TEST_ASSERT_TRUE_MESSAGE(luma(s[2].color) > luma(s[0].color), "so must the left edge");
    TEST_ASSERT_TRUE_MESSAGE(luma(s[3].color) < luma(s[0].color), "the bottom edge must be darker than the face");
    TEST_ASSERT_TRUE_MESSAGE(luma(s[4].color) < luma(s[0].color), "so must the right edge");
}
...
test_a_sunken_button_swaps_which_edges_are_lit(void) {
    ...
    for (int i = 1; i < UI_BEZEL_MAX_SPANS; i++) {
        ...
        TEST_ASSERT_TRUE_MESSAGE(same_color(sunken[i].color, raised[opposite].color),
...
test_panel_every_span_stays_inside_the_rect(void) {
    ...
    for (int i = 0; i < n; i++) {
        TEST_ASSERT_TRUE_MESSAGE(s[i].rect.x >= PANEL_X && s[i].rect.y >= PANEL_Y,
                                 "a span started outside the panel's rect");
        TEST_ASSERT_TRUE_MESSAGE(s[i].rect.x + s[i].rect.w <= PANEL_X + PANEL_W
...
test_panel_a_rect_too_small_for_a_border_gets_a_flat_face(void) {
    ...
    TEST_ASSERT_TRUE_MESSAGE(same_color(s[0].color, PANEL_FACE), "and that fill is the face colour, unshaded");
}
...
test_panel_a_zero_sized_rect_produces_nothing(void) {
    ...
    TEST_ASSERT_EQUAL_INT(0, ui_panel_spans((mu_Rect){4, 4, 20, 0}, PANEL_FACE, PANEL_BORDER, s, UI_PANEL_MAX_SPANS));
}
...
static void
poison(ui_text_pass_t* out, int n) {
    for (int i = 0; i < n; i++) {
...
static bool
is_poisoned(const ui_text_pass_t* p) {
    return p->dx == TEXT_SENTINEL && p->dy == TEXT_SENTINEL;
}

static void
test_plain_is_one_ink_pass_at_the_origin(void) {
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(1, n, "plain text is a single pass - no halo at all");
    TEST_ASSERT_EQUAL_INT(0, p[0].dx);
    TEST_ASSERT_EQUAL_INT(0, p[0].dy);
    TEST_ASSERT_TRUE_MESSAGE(p[0].ink, "the one pass plain text has must be the glyph itself, not a halo");
}

static void
test_outlined_is_eight_halo_offsets_and_the_ink(void) {
    ui_text_pass_t p[UI_TEXT_MAX_PASSES];
    poison(p, UI_TEXT_MAX_PASSES);

    const int n = ui_text_passes(UI_TEXT_OUTLINED, p, UI_TEXT_MAX_PASSES);

    TEST_ASSERT_EQUAL_INT_MESSAGE(UI_TEXT_MAX_PASSES, n, "an outline is eight halo offsets plus the glyph itself");
    ...
    for (int i = 0; i < 8; i++) {
        TEST_ASSERT_FALSE_MESSAGE(p[i].ink, "every pass before the last one must be halo, not ink");
        ...
        TEST_ASSERT_TRUE_MESSAGE(p[i].dx >= -1 && p[i].dx <= 1 && p[i].dy >= -1 && p[i].dy <= 1,
...
static void
test_shadowed_is_one_halo_offset_and_the_ink(void) {
    ui_text_pass_t p[UI_TEXT_MAX_PASSES];
    poison(p, UI_TEXT_MAX_PASSES);

    const int n = ui_text_passes(UI_TEXT_SHADOWED, p, UI_TEXT_MAX_PASSES);

    TEST_ASSERT_EQUAL_INT_MESSAGE(2, n, "a drop shadow is one halo offset plus the glyph itself");
...
static void
test_the_ink_pass_is_always_last(void) {
    ...
    for (size_t s = 0; s < sizeof(styles) / sizeof(styles[0]); s++) {
        ...
        TEST_ASSERT_TRUE_MESSAGE(p[n - 1].ink, "the ink pass must be last for every style, checked in a loop so "
                                               "a style added later cannot quietly break this - the glyph has "
                                               "to land on top of its own halo or it disappears under it");
        for (int i = 0; i < n - 1; i++) {
            TEST_ASSERT_FALSE_MESSAGE(p[i].ink, "no pass before the last one may be ink either, or the halo "
...
static void
test_a_text_buffer_too_small_produces_nothing(void) {
    const struct {
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
        ...
        for (int j = 0; j < UI_TEXT_MAX_PASSES; j++) {
            TEST_ASSERT_TRUE_MESSAGE(is_poisoned(&p[j]), "a refused call must not write anything into the buffer, "
...
static void
test_halo_of_a_dark_ink_is_light_and_well_separated(void) {
...
static void
test_halo_of_a_light_ink_is_dark_and_well_separated(void) {
...
static void
test_halo_carries_alpha_through_unchanged(void) {
    ...
    TEST_ASSERT_EQUAL_UINT8_MESSAGE(64, ui_text_halo(translucent_light).a, "same for a light, translucent ink");
}
...
test_a_control_too_small_for_a_bezel_gets_a_flat_face(void) {
    ...
    TEST_ASSERT_TRUE_MESSAGE(same_color(s[0].color, FACE), "and that fill is the face colour, unshaded");
}
...
test_a_zero_sized_control_produces_nothing(void) {
    ...
    TEST_ASSERT_EQUAL_INT(0, ui_bezel_spans((mu_Rect){4, 4, 20, 0}, FACE, false, s, UI_BEZEL_MAX_SPANS));
}

...
static void
test_mul_floor_matches_a_hand_written_widened_shift(void) {
    static const struct {
        int32_t a, b;
        int shift;
        const char* msg;
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
        const int64_t expect = ((int64_t)cases[i].a * (int64_t)cases[i].b) >> cases[i].shift;
...
static void
test_floor_and_round_diverge_on_an_inexact_negative_product(void) {
    ...
    TEST_ASSERT_EQUAL_INT32_MESSAGE(-1, fx_mul_round(-300, 1, 8), "-300/256 = -1.171875, nearest is -1");
}
...
static void
test_mul_round_rounds_to_nearest_for_positives(void) {
...
static void
test_floor_and_round_agree_on_exact_multiples(void) {
    static const struct {
        int32_t a, b;
        int shift;
        int32_t expect;
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
        ...
        TEST_ASSERT_EQUAL_INT32_MESSAGE(cases[i].expect, f, "an exact multiple has nothing to floor away");
        TEST_ASSERT_EQUAL_INT32_MESSAGE(cases[i].expect, r, "an exact multiple has nothing to round away");
    }
}
...
test_mul_floor_survives_a_32_bit_overflowing_product(void) {
    ...
    TEST_ASSERT_EQUAL_INT32_MESSAGE(8388626, expect, "sanity-check the hand-computed expectation before trusting it");
...
static void
test_mul_round_survives_a_32_bit_overflowing_product(void) {
...
static void
test_the_same_helpers_serve_both_shift_8_and_shift_16(void) {
